...
class User():
    def __init__(self,username):
        ...
        #Tạo đường dẫn
        self.path='D:/MainGame/'+self.username+'_history'
    def create_player_history(self):#tạo một lần duy nhất
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.MOUSEBUTTONDOWN:
                ...
                if profile_bt.is_in(spot[0],spot[1]):
                    ...
                    running_menu=True
                if temp_bt.is_in(spot[0],spot[1]):
                    running_menu=False
                    pygame.mouse.set_cursor(SYSTEM_CURSOR_ARROW)
            ...
            if start_bt.is_in(spot[0],spot[1]):
                ...
                pygame.mouse.set_cursor(SYSTEM_CURSOR_HAND)
            elif temp_bt.is_in(spot[0],spot[1]):
                pygame.mouse.set_cursor(SYSTEM_CURSOR_HAND)
...
def ranked_rs(image,width,height,player,com1,com2,com3,com4,user):
    ...
    while running_rank:
        ...
        for event in pygame.event.get():
            ...
            if home_bt.is_in(spot[0],spot[1]):
                pygame.mouse.set_cursor(SYSTEM_CURSOR_HAND)
                home_bt.font.set_underline(True)
            elif save_bt.is_in(spot[0],spot[1]):
                pygame.mouse.set_cursor(SYSTEM_CURSOR_HAND)
            else :
                pygame.mouse.set_cursor(SYSTEM_CURSOR_ARROW)
                home_bt.font.set_underline(False)
        ...
        home_bt.draw()
        save_bt.draw()
        draw_text('Home',home_bt.font,red,screen,home_bt.x+(screen_size[0]/16)/2,home_bt.y+(screen_size[1]/8)/2)
